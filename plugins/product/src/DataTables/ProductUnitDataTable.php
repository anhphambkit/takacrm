...
class ProductUnitDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('admin.product.unit.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, CUSTOMER_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('admin.product.unit.edit', 'admin.product.unit.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
        $model = app(ProductUnitRepositories::class)->getModel();
        /**
         * @var \Eloquent $model
         */
        $query = $model->select(
    ...
    public function buttons()
    {
        $buttons = [
            'create' => [
                'link' => route('admin.product.unit.create'),
