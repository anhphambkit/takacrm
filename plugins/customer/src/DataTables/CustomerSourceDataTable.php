...
class CustomerSourceDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('admin.customer.customer_source.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, CUSTOMER_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('admin.customer.customer_source.edit', 'admin.customer.customer_source.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
        $model = app(CustomerSourceRepositories::class)->getModel();
        /**
         * @var \Eloquent $model
         */
        $query = $model->select(
            [
                ...
                'customer_sources.created_by',
                'customer_sources.created_at',
                'customer_sources.status'
            ]);
        return $query;
    }
    ...
    public function columns()
    {
        return [
            ...
            'created_at' => [
                'name' => 'customer_sources.created_at',
    ...
    public function buttons()
    {
        $buttons = [
            'create' => [
                'link' => route('admin.customer.customer_source.create'),
