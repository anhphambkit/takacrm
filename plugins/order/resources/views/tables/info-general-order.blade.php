    ...
    <tbody>
        <tr>
            ...
            <td class="width-50-per">
                ...
                <span class="sub-content">{{ $order->user_performed_info['full_name'] }}</span>
            </td>
        </tr>
        <tr>
            <td class="width-50-per">
            ...
            <td class="width-50-per">
                ...
                <span class="sub-content">{{ ($order->user_performed_info['super_user']) ? \Core\User\Contracts\UserConfigs::SUPER_ADMIN : ($order->user_performed_info['get_role'][0]['name']) }}</span>
            </td>
        </tr>
        <tr>
            <td class="width-50-per">
        ...
        <tr>
            <td class="width-50-per">
                ...
                <span class="sub-content">{{ $order->customer_contact_info ? $order->customer_contact_info['full_name'] : "" }}</span>
            </td>
            <td class="width-50-per">
                <span class="title-sub-content text-bold-700">{{ trans('plugins-order::order.form.payment_method') }}: </span>
                <span class="sub-content">{{ $order->payment_method_info ? $order->payment_method_info['name'] : ""}}</span>
            </td>
        </tr>
    </tbody>
</table>
